...
internal sealed class DestekSmokeRunner(string baseUrl, string username, string password, bool allowStateMutations, bool prepareDemoData, int targetYear, int targetMonth, string targetTip)
{
    ...
    public async Task<int> RunAsync()
    {
        ...
        await ExecuteAsync("Aktif firma bağlamı hazırlanır", () => EnsureFirmaContextAsync(page));
    ...
    private async Task EnsureFirmaContextAsync(IPage page)
    {
        await page.GotoAsync($"{_baseUrl}/firma-sec?returnUrl=%2Fdashboard");
        await WaitForPageSettledAsync(page);

        if ((page.Url ?? string.Empty).Contains("/login", StringComparison.OrdinalIgnoreCase))
        {
            throw new SmokeSkippedException("Aktif firma bağlamı için giriş oturumu doğrulanamadı.");
        }

        var aktifFirmaYok = await page.Locator("text=Aktif firma bulunamadı.").First.IsVisibleAsync();
        if (aktifFirmaYok)
        {
            throw new SmokeSkippedException("Aktif firma bağlamı hazırlanamadı: sistemde aktif firma bulunamadı.");
        }

        var firmaKartlari = page.Locator("button.firma-card");
        var kartSayisi = await firmaKartlari.CountAsync();

        if (kartSayisi > 0)
        {
            await firmaKartlari.First.ClickAsync();
            await WaitForPageSettledAsync(page);
        }

        if ((page.Url ?? string.Empty).Contains("/firma-sec", StringComparison.OrdinalIgnoreCase))
        {
            throw new SmokeSkippedException("Aktif firma seçimi tamamlanamadı; firma seçim ekranında kalındı.");
        }
    }
    ...
    private async Task VerifyDeterministicHakedisDistributionAsync(IPage page)
    {
        ...
        await ApplyDeterministicHakedisFiltersAsync(page);
        ...
        var atlananMatch = Regex.Match(basariText, @"Atlanan:\s*(\d+)", RegexOptions.IgnoreCase);
        var hataliMatch = Regex.Match(basariText, @"Hatalı:\s*(\d+)", RegexOptions.IgnoreCase);

        if (!uretilenMatch.Success || !atlananMatch.Success || !hataliMatch.Success)
        {
        ...
        var atlananAdet = int.Parse(atlananMatch.Groups[1].Value);
        var hataliAdet = int.Parse(hataliMatch.Groups[1].Value);
        ...
        if (hataliAdet > 0)
        {
            throw new InvalidOperationException($"Deterministik dağılım için Toplu Hakediş Üret adımında hatalı kayıt olmamalı. Hatalı: {hataliAdet}, Atlanan: {atlananAdet}, Üretilen: {uretilenAdet}");
        }
        ...
        Console.WriteLine($"[INFO] Demo seed dağılım kontrolü ({_targetTip}/{hedefYil}-{hedefAy}, Üretilen={uretilenAdet}, Atlanan={atlananAdet}, Hatalı={hataliAdet}, MaxIdÖnce={oncekiMaxId}, SatırSonra={sonrakiIds.Count}): Taslak={taslakCount}, Onaylandi={onayliCount}, Badge={badgeCount}");
    }
    ...
    private async Task VerifyPuantajGridAsync(IPage page)
    {
        await page.GotoAsync($"{_baseUrl}/personel/puantaj-grid");
        await WaitForPageSettledAsync(page);
        await SkipIfAccessDeniedAsync(page, "Puantaj grid ekranı", "/personel/puantaj-grid");
        await EnsureAnyVisibleAsync(page,
            "text=Puantaj Çalışma Grid'i",
            "text=Excel Preview",
            "text=Önizleme");
    }

    private async Task VerifyPuantajGridExcelDefaultStateAsync(IPage page)
    {
        await page.GotoAsync($"{_baseUrl}/personel/puantaj-grid");
        await WaitForPageSettledAsync(page);
        await SkipIfAccessDeniedAsync(page, "Puantaj grid Excel aksiyonları", "/personel/puantaj-grid");

        var fileInput = page.Locator("input[type='file']").First;
        await fileInput.WaitForAsync();

        var previewButton = page.Locator("button:has-text('Önizleme')").First;
        var importButton = page.Locator("button:has-text('Import')").First;

        if (await previewButton.IsEnabledAsync())
        {
            throw new InvalidOperationException("Puantaj grid ekranında Önizleme butonu dosya seçilmeden pasif olmalı.");
        }

        if (await importButton.IsEnabledAsync())
        {
            throw new InvalidOperationException("Puantaj grid ekranında Import butonu dosya seçilmeden pasif olmalı.");
        }
    }
    ...
    private async Task VerifyOperationalPuantajFiltersAsync(IPage page)
    {
        await page.GotoAsync($"{_baseUrl}/operasyon/puantaj");
        await WaitForPageSettledAsync(page);
        await SkipIfAccessDeniedAsync(page, "Operasyonel puantaj filtreleri", "/operasyon/puantaj");
    ...
    private async Task VerifyOperationalHakedisAsync(IPage page)
    {
        await page.GotoAsync($"{_baseUrl}/operasyonel-hakedis");
        await WaitForPageSettledAsync(page);
        await SkipIfAccessDeniedAsync(page, "Operasyonel hakediş ekranı", "/operasyonel-hakedis");
        await EnsureAnyVisibleAsync(page,
    ...
    private async Task VerifyOperationalHakedisPeriodValidationAsync(IPage page)
    {
        await page.GotoAsync($"{_baseUrl}/operasyonel-hakedis");
        await WaitForPageSettledAsync(page);
        await SkipIfAccessDeniedAsync(page, "Operasyonel hakediş dönem validasyonu", "/operasyonel-hakedis");
    ...
    private async Task VerifyOperationalPuantajBulkActionsDefaultStateAsync(IPage page)
    {
        await page.GotoAsync($"{_baseUrl}/operasyon/puantaj");
        await WaitForPageSettledAsync(page);
        await SkipIfAccessDeniedAsync(page, "Operasyonel puantaj toplu aksiyon varsayılan state", "/operasyon/puantaj");
    ...
    private async Task VerifyOperationalHakedisBulkActionsDefaultStateAsync(IPage page)
    {
        await page.GotoAsync($"{_baseUrl}/operasyonel-hakedis");
        await WaitForPageSettledAsync(page);
        await SkipIfAccessDeniedAsync(page, "Operasyonel hakediş toplu aksiyon varsayılan state", "/operasyonel-hakedis");
    ...
    private async Task ApplyDeterministicHakedisFiltersAsync(IPage page)
    {
        var hedefYil = _targetYear.ToString();
        var hedefAy = _targetMonth.ToString();
    ...
    private static async Task SkipIfAccessDeniedAsync(IPage page, string scenario, string expectedPath)
    {
        var erisimReddedildi = await page.Locator("text=Erişim Reddedildi").First.IsVisibleAsync();
        var yetkiYokMetni = await page.Locator("text=gerekli yetkiniz bulunmuyor").First.IsVisibleAsync();

        var url = page.Url ?? string.Empty;
        var beklenenRotaDisi = !string.IsNullOrWhiteSpace(expectedPath)
                               && !url.Contains(expectedPath, StringComparison.OrdinalIgnoreCase);

        if (erisimReddedildi || yetkiYokMetni || beklenenRotaDisi)
        {
            throw new SmokeSkippedException($"{scenario} için kullanıcı yetkisi/erişimi uygun olmadığından senaryo atlandı. URL: {url}");
        }
    }
